...
(function() {
    ...
    $modal.on("shown.bs.modal", function() {
        if (initialized) {
			...
			originalData = $image.cropper("getCroppedCanvas"); // Save the data on hide
			
			formData.append('croppedImage', originalData);

			$.ajax('http://ibxprotocol.mxspecials.com/tools/test6', {
				method: "POST",
				data: formData,
				processData: false,
				contentType: false,
